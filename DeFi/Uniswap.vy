...
def swap(tokenIn: address, tokenOut: address, amountIn: uint256):
    ...
    # have to use low level function raw_call because Vyper does not support dynamic arrays
    res: Bytes[128] = raw_call(
        UNISWAP,
        concat(
            method_id("swapExactTokensForTokens(uint256,uint256,address[],address,uint256)"), # method id (first 4 bytes hash of func signature)
            ...
            convert(block.timestamp, bytes32), # deadling
            # handcraft dynamic array
            convert(3, bytes32), # path[] length
            convert(tokenIn, bytes32),
            convert(WETH, bytes32),
            convert(tokenOut, bytes32)
        ),
        max_outsize=128 # maximum size of output returned from calling the function
    )